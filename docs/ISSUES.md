...
| `.harness/` task record | one session's unit of work: kind, completion condition, premises, review verdict | the harness |
...
Stages follow the harness task record: candidate → opened with
`make task start` → closed with `make done` → accepted by the owner.
...
| next | agreed candidate waiting its turn | owner agrees it is wanted | owner approves the order → agent opens a task → now |
| now | in progress (a harness task is open) | `make task start` (an [着手] comment names the task id) | `make done` (verify green, review pass, premises settled) → awaiting acceptance |
| awaiting acceptance | implemented; the owner's hands-on acceptance remains | agent passed `make done` | **owner only** marks it complete (green tests are not acceptance) |
| complete | accepted | owner | — |

Kinds: **backlog** (candidates, later/next), **kanban** (open and just-closed
harness tasks), **operations** (recurring upkeep such as OS/toolchain updates,
...
- One task = one increment that reaches `make verify` green in one session;
  ...
  the body is brought to the shape in §3 and unconfirmed readings are copied
  into the task record's premises.
...
- Numbers: none beyond the tracker's own id and URL. Task names start with a
  kind tag — `[機能]` `[試す]` `[確認]` `[運用]` `[規則]`. R-xxx, D-xx and the
  harness task id are written alongside, each from its own canon.
...
and copied into the task record's premises when work starts.
...
- **Start**: before touching code, comment `[着手] <date> / branch or worktree
  / harness task id`. For size M or L, or a contested reading, comment
  `[計画] scope, approach, files, checks` first and wait for the owner's ok in
  a comment. Size S proceeds on the [着手] comment alone.
- **End / handoff**: comment `[完了] what was done, commits, verify tree,
  review verdict, what remains`, and move the stage to awaiting acceptance when
  `make done` passed. Stopping midway: `[中断] remainder and next step`.
